...
const CreateTrip = () => {
  ...
  const OnGenerateTrip = async () => {
    ...
    if (!formData?.noOfDays || !formData?.location || !formData?.budget || !formData?.traveler) {
      toast("Please fill all the details to generate a trip.");
      return;
    }
  
    // Check for number of days
    if (formData?.noOfDays > 10) {
      toast("Please select a duration shorter than 10 days to proceed.");
      return;
    }
  
    if (!formData?.month) {
      toast("Please select the month you plan to travel.");
      return;
    }
  
    if (formData?.noOfDays < 1) {
      toast("Trip duration must be at least 1 day. Please adjust your selection.");
      return;
    }
    ...
    <div className={`min-h-screen p-6 transition-all duration-300 ${isMonthModalOpen ? "filter blur-sm" : ""}`}>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-16 space-y-4">
          ...
          <div className="flex justify-center gap-4 text-red-600 opacity-75">
        ...
        {/* Destination Section */}
        <Card 
          className={`mb-8 bg-gradient-to-b from-white to-red-50 transition-all duration-600 hover:shadow-xl`}
        >
          <CardContent className="p-8">
            ...
            <div className="flex justify-center gap-8 mt-8">
              {locationIcons.map((Icon, index) => (
                <Icon 
                  key={index}
                  className="w-6 h-6 text-gray-400 transition-all duration-300 hover:text-red-500 hover:scale-110"
                />
              ))}
            </div>
          </CardContent>
        ...
        <Card   
          className={`mb-8 bg-gradient-to-b from-white to-red-50 transition-all duration-500 hover:shadow-xl`}
        >
          <CardContent className="p-8">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-3 bg-red-50 rounded-full">
        ...
        >
          <CardContent className="p-8">
            ...
            <Input
              ...
              min="1"
              max="10"
              className="border-gray-200 focus:ring-red-500 rounded-xl p-6 text-lg"
                ...
                <div
                  ...
                  className={`group cursor-pointer rounded-xl p-6 transition-all duration-300
                    ${formData?.budget === item.title 
                      ...
                      : 'bg-white hover:bg-red-50 border border-gray-200 hover:shadow-lg hover:scale-102'
                    }`}
                >
                  <div className="flex items-center gap-3 mb-3">
                    <item.icon className={`w-5 h-5 
                      ${formData?.budget === item.title ? 'text-white' : 'text-red-600'}`} 
                    />
                  ...
                  <p className={`text-sm
                    ${formData?.budget === item.title ? 'text-red-50' : 'text-gray-500'}`}>
                    {item.desc}
                  </p>
                </div>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {travelListWithIcons.map((item, index) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-3">
                    <item.icon className={`w-5 h-5 
                      ${formData?.traveler === item.people ? 'text-white' : 'text-red-600'}`} 
                    />
        ...
        <div className="flex justify-end gap-4 mt-12">
          ...
          <Button 
            ...
            disabled={loading}
            className="bg-red-600 hover:bg-red-600 text-white px-6 py-3 rounded-xl"
          >
            {loading ? (
