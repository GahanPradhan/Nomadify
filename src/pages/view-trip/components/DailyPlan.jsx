...
function DailyPlan({ trip }) {
  if (!trip?.tripData?.dailyPlans?.length) {
    return (
      <div className="mt-5">
        <h2 className="text-xl font-bold text-red-900 mb-4">Places to Visit</h2>
    ...
    <div className="w-full">
      ...
      <div className="space-y-6">
        {trip.tripData.dailyPlans.map((dayItem, dayIndex) => (
          <div 
            key={dayIndex} 
            className="bg-white border-2 border-red-100/50 rounded-3xl p-5 shadow-lg hover:shadow-xl transition-shadow duration-300"
          >
            <h2 className="font-bold text-lg text-red-600 mb-4 pb-2 border-b-2 border-red-100 flex items-center">
              <span className="bg-red-500 text-white px-3 py-1 rounded-full mr-3 text-sm">
                Day {dayIndex + 1}
              </span>
            </h2>
              ...
              {dayItem?.plan?.map((place, placeIndex) => (
                <div key={placeIndex} className="flex flex-col">
                  <div className="flex items-center mb-2">
                    {place.time && (
                      <div className="flex items-center mr-3 bg-red-50 text-red-600 px-2 py-1 rounded-full text-xs font-semibold">
                        <Clock className="mr-1" size={14} />
                        {formatTime(place.time)}
                      </div>
                    )}
                    <div className="flex-grow border-t-2 border-red-100/50"></div>
                  </div>
                  <CardItem 
                    place={place} 
                    day={dayIndex + 1} 
